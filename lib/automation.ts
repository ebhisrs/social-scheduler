...
function parseExtraData(raw: any): any {
  ...
  try { let p = JSON.parse(raw); if (typeof p === 'string') p = JSON.parse(p); return p || {} } catch { return {} }
}
...
async function sendFailureEmail(email: string, accountName: string, error: string, keyword: string) {
  try {
    ...
    const t = nodemailer.default.createTransport({ host: process.env.SMTP_HOST || 'live.smtp.mailtrap.io', port: parseInt(process.env.SMTP_PORT || '587'), auth: { user: smtpUser, pass: smtpPass } })
    await t.sendMail({ from: process.env.SMTP_FROM || smtpUser, to: email, subject: `Auto-post failed on ${accountName}`, html: `<p><b>Account:</b> ${accountName}</p><p><b>Keyword:</b> ${keyword}</p><p><b>Error:</b> ${error}</p>` })
  } catch (e: any) { console.error('[Email] Failed:', e.message) }
}

interface CompanyInfo { name?: string | null; phone?: string | null; address?: string | null; website?: string | null }

// Each trade has a list of service pairs. We pick a DIFFERENT pair every time
// a post is generated (random + avoid repeating the same pair two posts in a row
...
const SERVICES_MAP: Record<string, string[][]> = {
  ...
  serrurier: [
    ...
    ['ouverture porte', 'remplacement cylindre'],
  ],
  plombier: [
...
// Remember the last pair index picked for each schedule/trade so we don't repeat.
const lastPickByKey: Record<string, number> = {}
...
/**
 * Picks a random services pair for the given keyword, avoiding repeating the
 * same pair as the previous post for the same schedule.
 *
 * This replaces the old `getServicesForDay` which used the same pair for 24h,
 * which is why every post started the same way (e.g. "porte claquée...").
 */
function pickServices(keyword: string, scheduleId?: string): string[] {
  const tradeKey = getTradeKey(keyword)
  const pairs = SERVICES_MAP[tradeKey] || [['intervention rapide', 'devis gratuit']]
  if (pairs.length === 1) return pairs[0]

  const memoryKey = `${scheduleId || 'default'}:${tradeKey}`
  const last = lastPickByKey[memoryKey]

  let idx = Math.floor(Math.random() * pairs.length)
  // If we happened to pick the same one as last time, shift by one.
  if (last !== undefined && idx === last) idx = (idx + 1) % pairs.length

  lastPickByKey[memoryKey] = idx
  return pairs[idx]
}

export async function generateArticle(keyword: string, language: string, tone: string, company: CompanyInfo, scheduleId?: string): Promise<string> {
  const services = pickServices(keyword, scheduleId)
  ...
  // Seed the prompt with a random angle so the model doesn't fall into the
  // same "Service X + service Y..." opening every time.
  const openings = [
    'commence par une question au lecteur',
    'commence par une situation concrète du quotidien',
    'commence par une statistique ou un chiffre surprenant',
    'commence par une anecdote courte',
    'commence par un conseil pratique',
    'commence par une phrase qui crée l\'urgence',
  ]
  ...
  const raw = await aiChat([
    {
      role: 'system',
      content: `Tu es un rédacteur publicitaire. Tu écris en ${language}. Tu produis directement le texte final sans commentaires, sans explication, sans balises.`,
    },
    {
      role: 'user',
      content: `Écris un post publicitaire en ${language} pour un ${keyword}.
Services à mettre en avant (les intégrer naturellement dans le texte, PAS dans la première phrase) : ${services.join(', ')}.
${city ? `Ville : ${city}.` : ''}
Ton : ${tone}.
Style : ${opening}. Ne commence JAMAIS la première phrase par le nom d'un service.
Varie le vocabulaire d'un post à l'autre. (ref: ${variationSeed})
${hasCompany ? `Mentionne l'entreprise UNIQUEMENT à la toute fin : ${companyClosing}` : ''}
Format : 3 paragraphes courts. Appel à l'action à la fin. 600 à 800 caractères. Pas de hashtags dans le texte.`,
    },
  ], 0.95)

  const postText = cleanAIOutput(raw.trim())
  const final = buildFinalPost(postText, hashtags)
  console.log(`[Auto] services=[${services.join(',')}] opening="${opening}" | ${postText.length} chars | ${hashtags.length} hashtags`)
  return final
}
...
async function getRandomPhoto(): Promise<string[]> {
  ...
  if (cloudName && apiKey && apiSecret) {
    try {
      ...
      const data = await res.json(); const images = data.resources || []
      if (images.length) { const r = images[Math.floor(Math.random() * images.length)]; return [r.secure_url] }
    } catch {}
  }
  try {
    const { readdir } = await import('fs/promises'); const { existsSync } = await import('fs')
    const uploadDir = path.join(process.cwd(), 'public', 'uploads')
    if (!existsSync(uploadDir)) return []
    const files = await readdir(uploadDir); const images = files.filter(f => /\.(jpg|jpeg|png|webp|gif)$/i.test(f))
...
export async function processAutomation() {
  ...
  for (const schedule of schedules) {
    ...
    const matchingTime = timesForToday.find(t => { const [h, m] = t.split(':').map(Number); return Math.abs((h * 60 + m) - (now.getHours() * 60 + now.getMinutes())) <= 1 })
    ...
    const keyword = schedule.keyword || 'general', language = config.language || 'French', tone = config.tone || 'professional'
    ...
    for (let i = 0; i < postsPerSlot; i++) {
      try {
        ...
        const successAccounts: string[] = []; let lastError = ''
        ...
        let socialContent: string | null = null, wpContent: string | null = null
        ...
        const article = await prisma.article.create({ data: { title: `${keyword} — ${now.toLocaleDateString()} #${i + 1}`, content: articleContent, topic: keyword, language, published: true } })

        for (const account of accounts) {
          const content = account.platform === 'wordpress' ? (wpContent || articleContent) : (socialContent || articleContent)
          try {
            ...
            lastError = err.message; console.error(`[Auto] ❌ ${account.username}:`, err.message)
